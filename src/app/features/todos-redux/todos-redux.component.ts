...
export class TodosReduxComponent implements OnInit {
  ...
  constructor(private store: Store<TodosState>) {} // is called by angular as soon as component is created


  ngOnInit() {
    this.message$ = this.store.select(selectHeaderMessage);
    // in above line this.message$ is now subscriber for the store by using select statement
